  ...
  namespace reconstruction {
                                             ...
                                             snemo::datamodel::particle_track_data & track_)
    {
      ...
      for (gt::event::calorimeter_collection_type::const_iterator
        ...
        for (gt::event::calorimeter_collection_type::const_iterator
               jcalo = next(icalo); jcalo != the_gamma_calos.end(); ++jcalo) {
          ...
          // const double tof_prob = gt::tof_computing::get_proba(tof_chi2, the_gamma_calos.size() - 1);
          const double tof_prob = gt::tof_computing::get_proba(tof_chi2, 1);
      ...
      const std::list<std::list<int> > gamma_tracked_coll = gt.get_reflects(1e-2);
